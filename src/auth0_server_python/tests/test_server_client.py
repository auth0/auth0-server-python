...
async def test_server_client_mfa_property():
    """Verify server_client.mfa returns an MfaClient instance."""
    ...
    assert client.mfa._domain == "auth0.local"
    assert client.mfa._client_id == "<client_id>"


@pytest.mark.asyncio
async def test_get_access_token_mfa_required(mocker):
    """
    When token refresh returns mfa_required, get_access_token should raise
    MfaRequiredError with an encrypted mfa_token.
    """
    ...
    mock_state_store.get.return_value = {
        ...
        "internal": {"sid": "sid123", "created_at": 1000}
    }
    ...
    client = ServerClient(
        ...
        state_store=mock_state_store,
        transaction_store=AsyncMock(),
        secret="some-secret-long-enough-for-encryption-test"
    )
    ...
    # Mock token endpoint to return mfa_required
    response = AsyncMock()
    response.status_code = 403
    response.json = MagicMock(return_value={
        "error": "mfa_required",
        "error_description": "Multifactor authentication required",
        "mfa_token": "raw_mfa_token_from_auth0"
    })
    ...
    # The mfa_token should be encrypted (not the raw value)
    assert exc.value.mfa_token != "raw_mfa_token_from_auth0"
    assert isinstance(exc.value.mfa_token, str)
    assert len(exc.value.mfa_token) > 0
